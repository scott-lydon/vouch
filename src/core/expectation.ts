...
function buildBatchedVerifyPrompt(inputs: VerifyInputs[]): string {
  ...
  return [
    ...
    `- reasoning is one sentence. If false, name the specific divergence (e.g. "Expected the success toast; observed the error banner with text X").`,
    `- Be strict. A permutation that should have shown a success message but instead showed nothing is match=false, not match=true-with-caveat.`,
    ``,
